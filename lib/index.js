...
*/
/** dsh-internet-access cookie 名。 */
const IA_COOKIE = "dsh_public_access";

/** 计算 DSH home（$DSH_HOME 优先，回退 ~/.dsh）。 */
function iaDshHome() {
  const raw = process.env.DSH_HOME;
  if (raw !== undefined && raw.trim() !== "") return raw.trim().replace(/^~(?=$|\/)/, homedir());
  // 优先 $HOME（systemd 设 HOME=/home/admin），无则回退 homedir()
  const envHome = process.env.HOME;
  if (envHome !== undefined && envHome.trim() !== "") return join(envHome, ".dsh");
  return join(homedir(), ".dsh");
}

/** timing-safe 字符串比较（先哈希定长，再 timingSafeEqual）。 */
function iaSafeEqual(a, b) {
...
/**
 * 校验 dsh-internet-access 的会话/口令 cookie 是否真实有效。
 * 只有「访问口令」或「用户名+密码会话」二选一有效才放行。
 */
function iaValidAuthCookie(request) {
  const rawCookie = header(request.headers, "cookie") ?? "";
  const part = rawCookie.split(";").map((s) => s.trim()).find((s) => s.startsWith(`${IA_COOKIE}=`));
  if (part === undefined) return false;
  const value = part.slice(IA_COOKIE.length + 1);
  if (value === "") return false;
  // 1) 访问口令
  try {
    const tokenFile = join(iaDshHome(), "public-access-token.json");
    const tok = JSON.parse(readFileSync(tokenFile, "utf8"));
    if (typeof tok?.token === "string" && tok.token !== "" && iaSafeEqual(value, tok.token)) return true;
  } catch { /* 文件缺失/损坏 → 走会话 */ }
  // 2) 用户名+密码会话（过期删除）
  try {
    const sessionFile = join(iaDshHome(), "internet-access-sessions.json");
    const data = JSON.parse(readFileSync(sessionFile, "utf8"));
    const session = data?.sessions?.[value];
    if (session !== undefined && typeof session === "object") {
      if (Date.now() > (session.expiresAt ?? 0)) return false; // 过期
      return true;
    }
  } catch { /* 文件缺失/损坏 → 拒绝 */ }
  return false;
...
function isTrustedApiRequest(request, trustedHosts) {
	...
	// SECURITY HARDENING (by dsh-internet-access):
	// trustedHosts is a DNS-rebinding fence, NOT authentication. A public caller
	// whose Host is a trustedHosts entry (e.g. dsh.chunbosama.xyz) was previously
	// allowed straight through without any credential, which let attackers hit
	// /api/session.* /api/credentials.* directly. For NON-loopback requests we
	// now require a valid dsh-internet-access auth cookie (access token or
	// username+password session) before treating the caller as trusted.
	const isLoopback = isLoopbackHostname(hostUrl.hostname);
	if (!isLoopback && !isTrustedAuthority(hostUrl, trustedHosts)) return false;
	if (!isLoopback) {
		// Non-loopback caller must present a VALID (value-verified) internet-access
		// auth cookie — presence alone is not enough; the token/session is checked.
		if (!iaValidAuthCookie(request)) return false;
	}
