...
当公网入口（如 `dsh.chunbosama.xyz`）被加入 `--trusted-host` 后，攻击者只需把 `Host` 头写成受信任域名，即可**无认证**直接调用后端 `/api`，包括：
...
在 `isTrustedApiRequest` 中，对**非回环来源**（即公网请求）的 `/api` 访问，**强制要求携带真实有效的 dsh-internet-access 认证 cookie**（访问口令或用户名+密码会话），否则返回 403：

```javascript
// 原逻辑：只要 Host 是 trustedHosts 且非 cross-site 就放行（漏洞）
if (!isLoopbackHostname(hostUrl.hostname) && !isTrustedAuthority(hostUrl, trustedHosts)) return false;
if (header(request.headers, "sec-fetch-site") === "cross-site") return false;
const origin = header(request.headers, "origin");
if (origin === undefined) return true;   // ← 无认证直接放行
